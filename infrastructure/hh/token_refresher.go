...
// TokenRefresher реализует port.TokenRefresher для HeadHunter.
...
func (r *TokenRefresher) Refresh(ctx context.Context, cred entity.Credential, decryptionKey string) (string, int, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", "https://api.hh.ru/token", strings.NewReader(data.Encode()))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		return "", 0, fmt.Errorf("hh token API returned %d: %s", resp.StatusCode, body)
	}
